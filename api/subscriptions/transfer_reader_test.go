...
func TestTransferReader_Read(t *testing.T) {
	...
	// Act
	br := newTransferReader(thorNode.Chain().Repo(), genesisBlk.Header().ID(), filter)
	res, ok, err := br.Read()
...
func TestTransferReader_Read_NoNewBlocksToRead(t *testing.T) {
	...
	// Act
	br := newTransferReader(thorNode.Chain().Repo(), newBlock.Header().ID(), filter)
	res, ok, err := br.Read()
...
func TestTransferReader_Read_ErrorWhenReadingBlocks(t *testing.T) {
	...
	// Act
	br := newTransferReader(thorNode.Chain().Repo(), thor.MustParseBytes32("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), filter)
	res, ok, err := br.Read()
...
func TestTransferReader_Read_NoTransferMatchingTheFilter(t *testing.T) {
	...
	// Act
	br := newTransferReader(thorNode.Chain().Repo(), genesisBlk.Header().ID(), badFilter)
	res, ok, err := br.Read()
