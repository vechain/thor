...
import (
	...
	"github.com/vechain/thor/v2/p2psrv/discv5"
...
type Config struct {
	...
	StaticNodes []*tempdiscv5.Node

	// Trusted nodes are used as pre-configured connections which are always
	// allowed to connect, even above the peer limit.
	TrustedNodes []*tempdiscv5.Node
...
type Server struct {
	...
	quit          chan struct{}
	addstatic     chan *tempdiscv5.Node
	removestatic  chan *tempdiscv5.Node
	addtrusted    chan *tempdiscv5.Node
	removetrusted chan *tempdiscv5.Node
...
type conn struct {
	...
	id    tempdiscv5.NodeID // valid after the encryption handshake
...
type transport interface {
	// The two handshakes.
	doEncHandshake(prv *ecdsa.PrivateKey, dialDest *tempdiscv5.Node) (tempdiscv5.NodeID, error)
...
func (srv *Server) Peers() []*Peer {
	...
	// environments.
	case srv.peerOp <- func(peers map[tempdiscv5.NodeID]*Peer) {
		for _, p := range peers {
			ps = append(ps, p)
		}
	}:
...
func (srv *Server) PeerCount() int {
	var count int
	select {
	case srv.peerOp <- func(ps map[tempdiscv5.NodeID]*Peer) { count = len(ps) }:
		<-srv.peerOpDone
	case <-srv.quit:
	}
	return count
...
func (srv *Server) AddPeer(node *tempdiscv5.Node) {
	select {
...
func (srv *Server) RemovePeer(node *tempdiscv5.Node) {
	select {
...
func (srv *Server) AddTrustedPeer(node *tempdiscv5.Node) {
	select {
...
func (srv *Server) RemoveTrustedPeer(node *tempdiscv5.Node) {
	select {
...
func (srv *Server) Self() *tempdiscv5.Node {
	srv.lock.Lock()
	defer srv.lock.Unlock()

	if !srv.running {
		return &tempdiscv5.Node{IP: net.ParseIP("0.0.0.0")}
	}
	return srv.makeSelf(srv.listener, srv.ntab)
}

func (srv *Server) makeSelf(listener net.Listener, ntab discoverTable) *tempdiscv5.Node {
	// If the server's not running, return an empty node.
	// If the node is running but discovery is off, manually assemble the node infos.
	if ntab == nil {
		// Inbound connections disabled, use zero address.
		if listener == nil {
			return &tempdiscv5.Node{IP: net.ParseIP("0.0.0.0"), ID: tempdiscv5.PubkeyID(&srv.PrivateKey.PublicKey)}
		}
		// Otherwise inject the listener address too
		addr := listener.Addr().(*net.TCPAddr)
		return &tempdiscv5.Node{
			ID:  tempdiscv5.PubkeyID(&srv.PrivateKey.PublicKey),
			IP:  addr.IP,
...
type sharedUDPConn struct {
	*net.UDPConn
	unhandled chan tempdiscv5.ReadPacket
}
...
func (s *Server) KnownNodes() Nodes {
	nodes := make([]*tempdiscv5.Node, 0, len(s.TrustedNodes))
	//s.knownNodes.ForEach(func(ent *cache.PrioEntry) bool {
	//	nodes = append(nodes, ent.Value.(*tempdiscv5.Node))
	//	return true
	//})
	return nodes
}
...
func (srv *Server) Start(protocols []*Protocol, topic tempdiscv5.Topic) (err error) {
	...
	srv.addstatic = make(chan *tempdiscv5.Node)
	srv.removestatic = make(chan *tempdiscv5.Node)
	srv.addtrusted = make(chan *tempdiscv5.Node)
	srv.removetrusted = make(chan *tempdiscv5.Node)
	...
	if !srv.NoDiscovery && srv.DiscoveryV5 {
		unhandled = make(chan tempdiscv5.ReadPacket, 100)
		sconn = &sharedUDPConn{conn, unhandled}
	}

	// node table
	if !srv.NoDiscovery {
		ntab, err := tempdiscv5.ListenUDP(srv.PrivateKey, conn, realaddr, "", srv.NetRestrict, unhandled)
		if err != nil {
			return err
		}
		srv.TempDiscV5 = ntab
	...
	if srv.DiscoveryV5 {
		...
		srv.nodedb = db
		if sconn != nil {
			ntab, err = discv5.ListenV5(sconn, enode.NewLocalNode(srv.nodedb, srv.PrivateKey), cfg)
		} else {
			ntab, err = discv5.ListenV5(conn, enode.NewLocalNode(srv.nodedb, srv.PrivateKey), cfg)
		}
		if err != nil {
			return err
		}
...
type dialer interface {
	newTasks(running int, peers map[tempdiscv5.NodeID]*Peer, now time.Time) []task
	taskDone(task, time.Time)
	addStatic(*tempdiscv5.Node)
	removeStatic(*tempdiscv5.Node)
}
...
func (srv *Server) run(dialstate dialer) {
	...
	var (
		peers        = make(map[tempdiscv5.NodeID]*Peer)
		inboundCount = 0
		trusted      = make(map[tempdiscv5.NodeID]bool, len(srv.TrustedNodes))
...
running:
	for {
		...
		case n := <-srv.addstatic:
			...
			log.Trace("Adding static node", "node", n)
			dialstate.addStatic(n)
		case n := <-srv.removestatic:
			// This channel is used by RemovePeer to send a
			// disconnect request to a peer and begin the
			// stop keeping the node connected.
			log.Trace("Removing static node", "node", n)
		...
		case n := <-srv.addtrusted:
			...
			log.Trace("Adding trusted node", "node", n)
			trusted[n.ID] = true
		...
		case n := <-srv.removetrusted:
			...
			log.Trace("Removing trusted node", "node", n)
	...
	if srv.TempDiscV5 != nil {
		srv.TempDiscV5.Close()
	}
...
func (srv *Server) protoHandshakeChecks(peers map[tempdiscv5.NodeID]*Peer, inboundCount int, c *conn) error {
...
func (srv *Server) encHandshakeChecks(peers map[tempdiscv5.NodeID]*Peer, inboundCount int, c *conn) error {
	switch {
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag, dialDest *tempdiscv5.Node) error {
	self := srv.Self()
	if self == nil {
	...
	if err != nil {
		c.close(err)
		log.Trace("Setting up connection failed", "id", c.id, "err", err)
	}
	return err
}

func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *tempdiscv5.Node) error {
	...
	log.Trace("id", c.id, "addr", c.fd.RemoteAddr(), "conn", c.flags)
	// For dialed connections, check that the remote public key matches.
	if dialDest != nil && c.id != dialDest.ID {
		log.Trace("Dialed identity mismatch", "want", c, dialDest.ID)
		return DiscUnexpectedIdentity
	}
	err = srv.checkpoint(c, srv.posthandshake)
	if err != nil {
		log.Trace("Rejected peer before protocol handshake", "err", err)
		return err
	}
	// Run the protocol handshake
	phs, err := c.doProtoHandshake(srv.ourHandshake)
	if err != nil {
		log.Trace("Failed proto handshake", "err", err)
		return err
	}
	if phs.ID != c.id {
		log.Trace("Wrong devp2p handshake identity", "err", phs.ID)
		return DiscUnexpectedIdentity
	}
	c.caps, c.name = phs.Caps, phs.Name
	err = srv.checkpoint(c, srv.addpeer)
	if err != nil {
		log.Trace("Rejected peer", "err", err)
		return err
	}
	// If the checks completed successfully, runPeer has now been
	// launched by run.
	log.Trace("connection set up", "inbound", dialDest == nil)
	return nil
}
...
func (d *DiscV5DiscoveryTableWrapper) ReadRandomNodes(nodes []*tempdiscv5.Node) int {
	return d.inner.ReadRandomNodes(nodes)
}
