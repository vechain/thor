...
func init() {
	...
	defines := []struct {
		...
		{"native_energyAtBlock", func(env *xenv.Environment) []any {
			...
			if args.BlockNumber == ctx.Number {
				env.UseGas(thor.GetBalanceGas)
				val, err := Energy.Native(env.State(), ctx.Time, ctx.EnergyStopTime).Get(thor.Address(args.Self))
				if err != nil {
					panic(err)
				}
				return []any{val}
			...
			env.UseGas(thor.GetBalanceGas)
			val, err := Energy.Native(state, summary.Header.Timestamp(), env.BlockContext().EnergyStopTime).Get(thor.Address(args.Self))
			if err != nil {
				panic(err)
			}
