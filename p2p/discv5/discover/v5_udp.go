...
import (
	...
	"github.com/vechain/thor/v2/p2p/discv5/discover/v5wire"
...
func (t *UDPv5) send(toID enode.ID, toAddr netip.AddrPort, packet v5wire.Packet, c *v5wire.Whoareyou) (v5wire.Nonce, error) {
	...
	log.Trace(">> "+packet.Name(), t.logcontext...)
	return nonce, err
}
...
func (t *UDPv5) handlePacket(rawpacket []byte, fromAddr netip.AddrPort) error {
	...
	if err != nil {
		...
		//	t.unhandled <- up
		//	log.Error("Bad discv5 packet", "id", fromID, "addr", addr, "err", err)
		//	return nil
		//}
...
func (t *UDPv5) handleWhoareyou(p *v5wire.Whoareyou, fromID enode.ID, fromAddr netip.AddrPort) {
	...
	log.Trace("<< "+p.Name(), "id", c.node.ID(), "addr", fromAddr)
	c.handshakeCount++
	c.challenge = p
	p.Node = c.node
	t.sendCall(c)
