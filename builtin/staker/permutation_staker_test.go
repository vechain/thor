...
func NewAddValidationAction(
	...
	return NewActionBuilder("AddValidation").
		...
		WithExecute(
			func(ctx *ExecutionContext, s *StakerTest, blk int) error {
				if err := addContractVET(s, stake); err != nil {
					return err
				}
				if err := s.Staker.AddValidation(validator, endorser, period, stake); err != nil {
					subContractVET(s, stake)
		...
		WithCheck(
			func(ctx *ExecutionContext, s *StakerTest, blk int) error {
				val, err := s.Staker.GetValidation(validator)
				if err != nil {
...
func NewSignalExitAction(
	...
	return NewActionBuilder("SignalExit").
		WithMinParentBlocksRequired(minParentBlocksRequired).
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				if err := staker.Staker.SignalExit(validationID, endorserID, uint32(blk)); err != nil {
					return err
				}
		...
		WithCheck(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				val, err := staker.Staker.GetValidation(validationID)
				if err != nil {
...
func NewWithdrawAction(
	...
	return NewActionBuilder("Withdraw").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				amount, err := staker.Staker.WithdrawStake(validationID, endorserID, uint32(blk))
				if err != nil {
					return err
				}
				actualWithdrawnAmount = amount
				ctx.LastActionAmount = amount
				if err = subContractVET(staker, amount); err != nil {
					return err
				}
				return err
			}).
		WithCheck(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				_, err := staker.Staker.GetValidation(validationID)
				if err != nil {
...
func NewIncreaseStakeAction(
	...
	return NewActionBuilder("IncreaseStake").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				if err := addContractVET(staker, amount); err != nil {
					return err
				}
				if err := staker.Staker.IncreaseStake(validationID, endorserID, amount); err != nil {
					subContractVET(staker, amount)
					return err
				}
...
func NewDecreaseStakeAction(
	...
	return NewActionBuilder("DecreaseStake").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				if err := staker.Staker.DecreaseStake(validationID, endorserID, amount); err != nil {
					return err
				}
...
func NewSetBeneficiaryAction(
	...
	return NewActionBuilder("SetBeneficiary").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				return staker.Staker.SetBeneficiary(validationID, endorserID, beneficiary)
			}).
		WithCheck(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				val, err := staker.Staker.GetValidation(validationID)
...
func NewAddDelegationAction(
	...
	return NewActionBuilder("AddDelegation").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				if err := addContractVET(staker, stake); err != nil {
					return err
				}
				_, err := staker.Staker.AddDelegation(validationID, stake, multiplier, uint32(blk))
				if err != nil {
					subContractVET(staker, stake)
...
func NewSignalExitDelegationAction(
	...
	return NewActionBuilder("SignalExitDelegation").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				if err := staker.Staker.SignalDelegationExit(delegationID, uint32(blk)); err != nil {
					return err
				}
...
func NewWithdrawDelegationAction(
	...
	return NewActionBuilder("WithdrawDelegation").
		...
		WithExecute(
			func(ctx *ExecutionContext, staker *StakerTest, blk int) error {
				amount, err := staker.Staker.WithdrawDelegation(delegationID, uint32(blk))
				if err != nil {
...
func subContractVET(s *StakerTest, amount uint64) error {
	balance, err := s.state.GetBalance(s.Staker.Address())
	if err != nil {
		return err
	}
	newBalance := big.NewInt(0).Sub(balance, ToWei(amount))
	if err = s.state.SetBalance(s.Staker.Address(), newBalance); err != nil {
		return err
	}
	s.state.SetStorage(s.Staker.Address(), thor.Bytes32{}, thor.BytesToBytes32(newBalance.Bytes()))
	return nil
}

// addContractVET adds `amount` VET to the contract balance
func addContractVET(s *StakerTest, amount uint64) error {
	balance, err := s.state.GetBalance(s.Staker.Address())
	if err != nil {
		return err
	}
