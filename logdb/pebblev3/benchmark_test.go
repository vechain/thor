...
func BenchmarkPebbleV3_FilterEvents_SingleAddress(b *testing.B) {
	...
	filter := &logdb.EventFilter{
		...
		Range:   &logdb.Range{From: 1, To: 1000},
...
func BenchmarkPebbleV3_FilterEvents_SingleTopic(b *testing.B) {
...
func BenchmarkPebbleV3_FilterEvents_AddressAndTopic(b *testing.B) {
	db, cleanup := setupBenchmarkDB(b, 1000)
	defer cleanup()
...
func BenchmarkPebbleV3_FilterEvents_MultiCriteriaOR(b *testing.B) {
	db, cleanup := setupBenchmarkDB(b, 1000)
	defer cleanup()
...
func BenchmarkPebbleV3_FilterEvents_DESC_Order(b *testing.B) {
	db, cleanup := setupBenchmarkDB(b, 1000)
	defer cleanup()
	...
	filter := &logdb.EventFilter{
		...
		Range:   &logdb.Range{From: 1, To: 1000},
		Options: &logdb.Options{Offset: 0, Limit: 100},
		Order:   logdb.DESC, // Test DESC order performance
	}
...
func BenchmarkPebbleV3_FilterTransfers(b *testing.B) {
	db, cleanup := setupBenchmarkDB(b, 1000)
	defer cleanup()
...
func BenchmarkPebbleV3_Write(b *testing.B) {
	tmpDir, err := os.MkdirTemp("", "pebblev3_bench_write")
	if err != nil {
...
func BenchmarkPebbleV3_MemoryUsage(b *testing.B) {
	...
	filter := &logdb.EventFilter{
		...
		Range:   &logdb.Range{From: 1, To: 5000},
		Options: &logdb.Options{Offset: 0, Limit: 50}, // Small limit to test streaming
		Order:   logdb.ASC,
	}
...
func setupBenchmarkDB(b *testing.B, numBlocks int) (*PebbleV3LogDB, func()) {
	tmpDir, err := os.MkdirTemp("", "pebblev3_bench")
	if err != nil {
		b.Fatalf("Failed to create temp dir: %v", err)
	}
...
// === New Microbenchmarks for PebbleV3 Performance Analysis ===

// BenchmarkPebbleV3_AddressOnly_LargeRange tests address-only queries over large block ranges
func BenchmarkPebbleV3_AddressOnly_LargeRange(b *testing.B) {
	...
	filter := &logdb.EventFilter{
		...
		Range:   &logdb.Range{From: 1000, To: 9000}, // Large range: 8000 blocks
...
// BenchmarkPebbleV3_TopicOnly_LargeRange tests topic-only queries over large block ranges  
func BenchmarkPebbleV3_TopicOnly_LargeRange(b *testing.B) {
	resetMetrics() // Reset debug metrics
	defer func() {
		// Only log metrics if debug build (metrics != nil)
		if metrics := getMetrics(); metrics != nil {
...
// BenchmarkPebbleV3_MultiTopic_AND tests multi-topic intersection (AND logic)
func BenchmarkPebbleV3_MultiTopic_AND(b *testing.B) {
	resetMetrics() // Reset debug metrics
	defer func() {
		// Only log metrics if debug build (metrics != nil)
		if metrics := getMetrics(); metrics != nil {
	...
	filter := &logdb.EventFilter{
		...
		Range:   &logdb.Range{From: 1, To: 5000},
		Options: &logdb.Options{Offset: 0, Limit: 500},
		Order:   logdb.ASC,
	}
...
// BenchmarkPebbleV3_MassiveOR tests 40-criteria OR queries (stress test union)
func BenchmarkPebbleV3_MassiveOR(b *testing.B) {
	resetMetrics() // Reset debug metrics
	defer func() {
		// Only log metrics if debug build (metrics != nil)
		if metrics := getMetrics(); metrics != nil {
	...
	criteriaSet := make([]*logdb.EventCriteria, 40)
	for i := 0; i < 40; i++ {
		// Generate different addresses to ensure variety
		var addr [20]byte
		addr[0] = byte(i)      // Make each address different
		addr[1] = byte(i >> 8)
		address := thor.Address(addr)
		
		criteriaSet[i] = &logdb.EventCriteria{
			Address: &address,
		}
	}
	...
	filter := &logdb.EventFilter{
		...
		Range:       &logdb.Range{From: 1, To: 3000},
		Options:     &logdb.Options{Offset: 0, Limit: 1000},
		Order:       logdb.ASC,
	}
...
// BenchmarkPebbleV3_EmptyCriteriaSet tests range-only queries (empty CriteriaSet support)
func BenchmarkPebbleV3_EmptyCriteriaSet_Events(b *testing.B) {
	resetMetrics() // Reset debug metrics
	defer func() {
		// Only log metrics if debug build (metrics != nil)
		if metrics := getMetrics(); metrics != nil {
	...
	filter := &logdb.EventFilter{
		CriteriaSet: []*logdb.EventCriteria{}, // Empty CriteriaSet - range-only query
		Range:       &logdb.Range{From: 1000, To: 2000}, // 1000 blocks
		Options:     &logdb.Options{Offset: 0, Limit: 500},
		Order:       logdb.ASC,
	}
...
// BenchmarkPebbleV3_EmptyCriteriaSet_Transfers tests range-only transfer queries
func BenchmarkPebbleV3_EmptyCriteriaSet_Transfers(b *testing.B) {
	resetMetrics() // Reset debug metrics
	defer func() {
		// Only log metrics if debug build (metrics != nil)
		if metrics := getMetrics(); metrics != nil {
	...
	filter := &logdb.TransferFilter{
		CriteriaSet: []*logdb.TransferCriteria{}, // Empty CriteriaSet - range-only query
		Range:       &logdb.Range{From: 1000, To: 2000}, // 1000 blocks
		Options:     &logdb.Options{Offset: 0, Limit: 500},
		Order:       logdb.ASC,
	}
