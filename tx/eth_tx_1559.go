...
// Non-empty access lists are rejected by ParseEthTransaction (EthErrAccessListUnsupported)
...
type eth1559TxData struct {
	...
	value       *big.Int
	data        []byte
	accessList  []AccessListEntry // always nil/empty — rejected by ParseEthTransaction
	yParity     uint8
	r, s        *big.Int
...
// recovery) and the switch checks in validateBlockBody; the mempool runs
// ParseEthTransaction before any tx enters the pool.
