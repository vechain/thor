...
func (tab *Table) setFallbackNodes(nodes []*enode.Node) error {
	...
	for _, n := range nodes {
		...
		if tab.cfg.NetRestrict != nil && !tab.cfg.NetRestrict.Contains(netutil.AddrToIP(n.IPAddr())) {
...
func (tab *Table) addIP(b *bucket, ip netip.Addr) bool {
	...
	if !tab.ips.Add(netutil.AddrToIP(ip)) {
		tab.log.Debug("IP exceeds table limit", "ip", ip)
		return false
	}
	if !b.ips.Add(netutil.AddrToIP(ip)) {
		tab.log.Debug("IP exceeds bucket limit", "ip", ip)
		tab.ips.Remove(netutil.AddrToIP(ip))
		return false
	}
	return true
}
...
func (tab *Table) removeIP(b *bucket, ip netip.Addr) {
	...
	tab.ips.Remove(netutil.AddrToIP(ip))
	b.ips.Remove(netutil.AddrToIP(ip))
}
