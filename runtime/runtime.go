...
func (rt *Runtime) newEVM(stateDB *statedb.StateDB, clauseIndex uint32, txCtx *xenv.TransactionContext) *vm.EVM {
	...
	return vm.NewEVM(vm.Context{
		...
		Transfer: func(_ vm.StateDB, sender, recipient common.Address, amount *big.Int) {
			...
			senderEnergy, err := builtin.Energy.Native(rt.state, rt.ctx.Time, rt.ctx.EnergyStopTime).Get(thor.Address(sender))
			if err != nil {
				panic(err)
			}
			recipientEnergy, err := builtin.Energy.Native(rt.state, rt.ctx.Time, rt.ctx.EnergyStopTime).Get(thor.Address(recipient))
			if err != nil {
				panic(err)
			}
		...
		OnSuicideContract: func(_ *vm.EVM, contractAddr, tokenReceiver common.Address) {
			...
			energy, err := builtin.Energy.Native(rt.state, rt.ctx.Time, rt.ctx.EnergyStopTime).Get(thor.Address(contractAddr))
			if err != nil {
				panic(err)
			}
			...
			if bal.Sign() != 0 || energy.Sign() != 0 {
				receiverEnergy, err := builtin.Energy.Native(rt.state, rt.ctx.Time, rt.ctx.EnergyStopTime).Get(thor.Address(tokenReceiver))
				if err != nil {
					panic(err)
				}
...
func (rt *Runtime) PrepareTransaction(trx *tx.Transaction) (*TransactionExecutor, error) {
	...
	return &TransactionExecutor{
		...
		Finalize: func() (*tx.Receipt, error) {
			...
			if err := builtin.Energy.Native(rt.state, rt.ctx.Time, rt.ctx.EnergyStopTime).Add(rt.ctx.Beneficiary, receipt.Reward); err != nil {
				return nil, err
			}
			return receipt, nil
		},
