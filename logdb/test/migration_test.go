...
func TestMigration_DataIntegrity(t *testing.T) {
	...
	originalEvents, err := sqliteDB.FilterEvents(context.Background(), &logdb.EventFilter{
		Options: &logdb.Options{Limit: 1000},
	})
	require.NoError(t, err)

	originalTransfers, err := sqliteDB.FilterTransfers(context.Background(), &logdb.TransferFilter{
		Options: &logdb.Options{Limit: 1000},
	})
	require.NoError(t, err)
	...
	migratedEvents, err := pebbleDB.FilterEvents(context.Background(), &logdb.EventFilter{
		Options: &logdb.Options{Limit: 1000},
	})
	require.NoError(t, err)

	migratedTransfers, err := pebbleDB.FilterTransfers(context.Background(), &logdb.TransferFilter{
		Options: &logdb.Options{Limit: 1000},
	})
	require.NoError(t, err)
