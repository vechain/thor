...
func (t *jsTracer) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) {
	if t.err != nil {
		return
	}
...
func (t *jsTracer) SetContext(ctx *tracers.Context) {
	if ctx == nil {
	...
	if ctx.BlockID != (thor.Bytes32{}) {
		t.ctx["blockHash"] = t.vm.ToValue(ctx.BlockID.Bytes())
		if ctx.TxID != (thor.Bytes32{}) {
			t.ctx["txIndex"] = t.vm.ToValue(ctx.TxIndex)
			t.ctx["txHash"] = t.vm.ToValue(ctx.TxID.Bytes())
		}
	}
}
...
func (t *jsTracer) setBuiltinFunctions() {
	...
	jsvm.Set("toHex", func(v goja.Value) string {
		b, err := t.fromBuf(jsvm, v, false)
		if err != nil {
			jsvm.Interrupt(err)
			return ""
		}
	...
	jsvm.Set("toWord", func(v goja.Value) goja.Value {
		// TODO: add test with []byte len < 32 or > 32
		b, err := t.fromBuf(jsvm, v, true)
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
	...
	jsvm.Set("toAddress", func(v goja.Value) goja.Value {
		a, err := t.fromBuf(jsvm, v, true)
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
		a = common.BytesToAddress(a).Bytes()
		res, err := t.toBuf(jsvm, a)
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
	...
	jsvm.Set("toContract2", func(from goja.Value, salt string, initcode goja.Value) goja.Value {
		a, err := t.fromBuf(jsvm, from, true)
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
		addr := common.BytesToAddress(a)
		code, err := t.fromBuf(jsvm, initcode, true)
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
	...
	jsvm.Set("isPrecompiled", func(v goja.Value) bool {
		a, err := t.fromBuf(jsvm, v, true)
		if err != nil {
			jsvm.Interrupt(err)
			return false
	...
	jsvm.Set("slice", func(slice goja.Value, start, end int) goja.Value {
		...
		if start < 0 || start > end || end > len(b) {
			jsvm.Interrupt(fmt.Sprintf("Tracer accessed out of bound memory: available %d, offset %d, size %d", len(b), start, end-start))
			return nil
		}
		res, err := t.toBuf(jsvm, b[start:end])
		if err != nil {
			jsvm.Interrupt(err)
			return nil
		}
