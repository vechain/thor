...
func TestOpMcopy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "non-overlapping copy 32 bytes",
		...
		{
			name: "self copy",
			dst:  0, src: 0, length: 32,
			preData:  bytes.Repeat([]byte{0x01}, 32),
			wantData: bytes.Repeat([]byte{0x01}, 32),
		},
		{
			name: "overlapping src > dst",
		...
		{
			name: "overlapping dst > src",
			dst:  1, src: 0, length: 8,
			preData: []byte{
				0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
				0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		...
		t.Run(tc.name, func(t *testing.T) {
			mem := NewMemory()
			mem.Resize(uint64(len(tc.preData)))
			mem.Set(0, uint64(len(tc.preData)), tc.preData)

			stack := newstack()
			stack.push(new(uint256.Int).SetUint64(tc.length))
			stack.push(new(uint256.Int).SetUint64(tc.src))
			stack.push(new(uint256.Int).SetUint64(tc.dst))

			opMcopy(&pc, env, nil, mem, stack)

			if !bytes.Equal(tc.wantData, mem.Data()) {
				t.Errorf("want: %#x\nhave: %#x", tc.wantData, mem.Data())
			}
			returnStack(stack)
		})
	}
