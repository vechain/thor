...
import (
	...
	"github.com/vechain/thor/v2/p2p/discv5/netutil"
)
...
func (tab *Table) setFallbackNodes(nodes []*enode.Node) error {
	...
	for _, n := range nodes {
		...
		if tab.cfg.NetRestrict != nil && !tab.cfg.NetRestrict.ContainsAddr(n.IPAddr()) {
			log.Error("Bootstrap node filtered by netrestrict", "id", n.ID(), "ip", n.IPAddr())
			continue
		}
...
func (tab *Table) deleteInBucket(b *bucket, id enode.ID) *tableNode {
	...
	if len(b.replacements) == 0 {
		log.Debug("Removed dead node", "b", b.index, "id", n.ID(), "ip", n.IPAddr())
		return nil
	}
	...
	tab.nodeAdded(b, rep)
	log.Debug("Replaced dead node", "b", b.index, "id", n.ID(), "ip", n.IPAddr(), "r", rep.ID(), "rip", rep.IPAddr())
	return rep
}
