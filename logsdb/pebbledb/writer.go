...
var _ logsdb.Writer = (*PebbleDBWriter)(nil)

// PebbleDBWriter implements logsdb.Writer interface for PebbleDB v3
type PebbleDBWriter struct {
	db        *pebble.DB
	batch     *pebble.Batch
	batchSize int
}

// NewPebbleDBWriter creates a new writer instance
func NewPebbleDBWriter(db *pebble.DB) *PebbleDBWriter {
	return &PebbleDBWriter{
		db:    db,
		batch: db.NewBatch(),
	}
}
...
func (w *PebbleDBWriter) Write(b *block.Block, receipts tx.Receipts) error {
	...
	for txIdx, receipt := range receipts {
		...
		for _, output := range receipt.Outputs {
			...
			transferCount += len(output.Transfers)
		}
...
func (w *PebbleDBWriter) writeEvent(seq sequence, blockID thor.Bytes32, blockNum uint32, blockTime uint64,
	txID thor.Bytes32, txIdx uint32, txOrigin thor.Address, clauseIdx uint32, logIdx uint32, event *tx.Event,
) error {
	// Create event record
...
func (w *PebbleDBWriter) writeTransfer(seq sequence, blockID thor.Bytes32, blockNum uint32, blockTime uint64,
	txID thor.Bytes32, txIdx uint32, txOrigin thor.Address, clauseIdx uint32, logIdx uint32, transfer *tx.Transfer,
) error {
...
func (w *PebbleDBWriter) Rollback() error {
	w.batch.Reset()
	w.batchSize = 0
	return nil
}
...
func (w *PebbleDBWriter) UncommittedCount() int {
	return w.batchSize
}
...
func (w *PebbleDBWriter) Truncate(blockNum uint32) error {
...
func (w *PebbleDBWriter) truncateIndexPrefixChunked(prefix string, minSeq sequence) error {
...
func (w *PebbleDBWriter) isReceiptEmpty(r *tx.Receipt) bool {
...
func (w *PebbleDBWriter) extractTxInfo(b *block.Block, txIdx int) (thor.Bytes32, thor.Address) {
	txs := b.Transactions()
	if txIdx < len(txs) {
		tx := txs[txIdx]
		txID := tx.ID()
...
func (w *PebbleDBWriter) WriteMigrationEvents(events []*logsdb.Event) error {
...
func (w *PebbleDBWriter) WriteMigrationTransfers(transfers []*logsdb.Transfer) error {
