...
func (tab *Table) doRevalidate(resp revalidationResponse, node *enode.Node) {
	...
	if remoteSeq > node.Seq() {
		...
		if err != nil {
			log.Debug("ENR request failed", "id", node.ID(), "err", err)
		} else {
...
func (tr *tableRevalidation) handleResponse(tab *Table, resp revalidationResponse) {
	...
	if !resp.didRespond {
		...
		if n.livenessChecks <= 0 {
			...
			log.Debug("Node revalidation failed", "b", b.index, "id", n.ID(), "checks", n.livenessChecks, "q", n.revalList.name)
	...
	log.Debug("Node revalidated", "b", b.index, "id", n.ID(), "checks", n.livenessChecks, "q", n.revalList.name)
