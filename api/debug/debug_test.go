...
func testTraceClauseWithInvalidTracerName(t *testing.T) {
	res := httpPostAndCheckResponseStatus(t, "/debug/tracers", &types.TraceClauseOption{Name: "non-existent"}, 403)
	assert.Contains(t, res, "unable to create custom tracer")
}

func testTraceClauseWithEmptyTracerTarget(t *testing.T) {
	res := httpPostAndCheckResponseStatus(t, "/debug/tracers", &types.TraceClauseOption{Name: "structLogger"}, 400)
	assert.Equal(t, "target: unsupported", strings.TrimSpace(res))
}

func testTraceClauseWithBadBlockID(t *testing.T) {
	traceClauseOption := &types.TraceClauseOption{
		Name:   "structLogger",
...
func testHandleTraceCallWithEmptyTraceCallOption(t *testing.T) {
	traceCallOption := &types.TraceCallOption{Name: "structLogger"}
	expectedExecutionResult := &logger.ExecutionResult{
		Gas:         0,
		Failed:      false,
		ReturnValue: "",
...
func testHandleTraceCall(t *testing.T) {
	...
	traceCallOption := &types.TraceCallOption{
		Name:       "structLogger",
		To:         &addr,
...
func testHandleTraceCallWithValidRevisions(t *testing.T) {
	...
	for _, revision := range revisions {
		...
		res := httpPostAndCheckResponseStatus(t, "/debug/tracers/call?revision="+revision, &types.TraceCallOption{Name: "structLogger"}, 200)
...
func testHandleTraceCallWithRevisionAsNonExistingHeight(t *testing.T) {
	res := httpPostAndCheckResponseStatus(t, "/debug/tracers/call?revision=12345", &types.TraceCallOption{}, 400)
...
func testHandleTraceCallWithRevisionAsNonExistingID(t *testing.T) {
	...
	res := httpPostAndCheckResponseStatus(t, "/debug/tracers/call?revision="+nonExistingRevision, &types.TraceCallOption{}, 400)
...
func testStorageRangeWithError(t *testing.T) {
	...
	badMaxResult := &types.StorageRangeOption{MaxResult: 1001}
...
func testStorageRange(t *testing.T) {
	...
	expectedStorageRangeResult := &types.StorageRangeResult{
		Storage: make(types.StorageMap, 0),
		NextKey: nil,
	}

	res := httpPostAndCheckResponseStatus(t, "/debug/storage-range", &opt, 200)

	var parsedExecutionRes *types.StorageRangeResult
	if err := json.Unmarshal([]byte(res), &parsedExecutionRes); err != nil {
		t.Fatal(err)
	}
...
func testStorageRangeDefaultOption(t *testing.T) {
	...
	var storageRangeRes *types.StorageRangeResult
