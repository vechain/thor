...
func TestMigration_DataIntegrity(t *testing.T) {
	...
	originalEvents, err := sqlite3.FilterEvents(context.Background(), &logsdb.EventFilter{
		Options: &logsdb.Options{Limit: 1000},
	})
	require.NoError(t, err)

	originalTransfers, err := sqlite3.FilterTransfers(context.Background(), &logsdb.TransferFilter{
		Options: &logsdb.Options{Limit: 1000},
	})
	require.NoError(t, err)
