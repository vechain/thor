...
func defaultAction(ctx *cli.Context) error {
	...
	apiHandler, apiCloser := api.New(chain, state.NewCreator(mainDB), txPool, logDB, p2pcom.comm, ctx.String(apiCorsFlag.Name), uint32(ctx.Int(apiSubsOffsetFlag.Name)))
...
func soloAction(ctx *cli.Context) error {
	...
	apiHandler, apiCloser := api.New(chain, state.NewCreator(mainDB), txPool, logDB, solo.Communicator{}, ctx.String(apiCorsFlag.Name), uint32(ctx.Int(apiSubsOffsetFlag.Name)))
	defer func() { log.Info("closing API..."); apiCloser() }()
