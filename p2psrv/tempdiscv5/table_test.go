...
import (
	...
	"github.com/vechain/thor/v2/p2psrv/discover"
)
...
func (nullTransport) sendFindnode(remote *Node, target discover.NodeID)     {}
...
type pingRecorder struct{ responding, pinged map[discover.NodeID]bool }

func newPingRecorder() *pingRecorder {
	return &pingRecorder{make(map[discover.NodeID]bool), make(map[discover.NodeID]bool)}
}

func (t *pingRecorder) findnode(toid discover.NodeID, toaddr *net.UDPAddr, target discover.NodeID) ([]*Node, error) {
	panic("findnode called on pingRecorder")
}
func (t *pingRecorder) close() {}
func (t *pingRecorder) waitping(from discover.NodeID) error {
	return nil // remote always pings
}
func (t *pingRecorder) ping(toid discover.NodeID, toaddr *net.UDPAddr) error {
	t.pinged[toid] = true
	...
	test := func(test *closeTest) bool {
		...
		for _, b := range tab.buckets {
			for _, n := range b.entries {
				...
				if discover.DistCmp(test.Target, n.sha, farthestResult) < 0 {
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	...
	test := func(buf []*Node) bool {
		tab := newTable(discover.NodeID{}, &net.UDPAddr{})
		for i := 0; i < len(buf); i++ {
...
func (*closeTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &closeTest{
		Self:   gen(discover.NodeID{}, rand).(discover.NodeID),
		Target: gen(common.Hash{}, rand).(common.Hash),
		N:      rand.Intn(bucketSize),
	}
	for _, id := range gen([]discover.NodeID{}, rand).([]discover.NodeID) {
...
func sortedByDistanceTo(distbase common.Hash, slice []*Node) bool {
	...
	for i, e := range slice {
		if i > 0 && discover.DistCmp(distbase, e.sha, last) < 0 {
			return false
		}
		last = e.sha
	}
...
func contains(ns []*Node, id discover.NodeID) bool {
	for _, n := range ns {
		if n.ID == id {
			return true
		}
