...
func newStorage(addr thor.Address, state *state.State, charger *gascharger.Charger) *storage {
	root := solidity.NewRoot(addr, state, charger)
	return &storage{
		root:         root,
		validations:  solidity.NewMapping[thor.Bytes32, *Validation](root, slotValidations),
		aggregations: solidity.NewMapping[thor.Bytes32, *Aggregation](root, slotAggregations),
		delegations:  solidity.NewMapping[thor.Bytes32, *Delegation](root, slotDelegations),
		lookups:      solidity.NewMapping[thor.Address, thor.Bytes32](root, slotValidationLookups),
		rewards:      solidity.NewMapping[thor.Bytes32, *big.Int](root, slotRewards),
		exits:        solidity.NewMapping[*big.Int, thor.Bytes32](root, slotExitEpochs),
	}
}

func (s *storage) Root() *solidity.Root {
	return s.root
}
...
func (s *storage) debugOverride(ptr *uint32, bytes32 thor.Bytes32) {
	if num, err := solidity.NewUint256(s.Root(), bytes32).Get(); err == nil {
