...
func TestFlow_Revert(t *testing.T) {
	...
	failingTransaction := tx.NewBuilder(tx.TypeDynamicFee).
		...
		Clause(tx.NewClause(&builtin.Staker.Address).WithData([]byte{0xc3, 0xc4, 0xb1, 0x38, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			...
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0e, 0x10}).WithValue(amount)).
