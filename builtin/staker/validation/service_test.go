...
func TestService_ActivateAndExit_Flow(t *testing.T) {
	...
	renew, err := (&Validation{body: &body{QueuedVET: uint64(100), LockedVET: uint64(0), Weight: uint64(0)}}).renew(uint64(0))
	assert.NoError(t, err)
...
func TestService_LeaderGroup_ReturnsActiveOnly(t *testing.T) {
	...
	assert.NoError(t, svc.repo.addValidation(q, &Validation{&body{Status: StatusQueued}}))
	a := thor.BytesToAddress([]byte("a"))
	assert.NoError(t, svc.repo.addValidation(a, &Validation{&body{Status: StatusActive}}))
...
func TestService_IsActive_Flag(t *testing.T) {
	...
	idv := thor.BytesToAddress([]byte("x"))
	id := idv
...
func TestService_SignalExit_SetsExitBlockAndPersists(t *testing.T) {
	...
	assert.NoError(t, err)
	err = svc.SignalExit(id, 120, val.StartBlock()+val.Period()*current, 20)
	assert.NoError(t, err)
...
func TestService_SignalExit_ExitBlockLimitReached(t *testing.T) {
	...
	val := thor.BytesToAddress([]byte("validator"))
	...
	updatedVal, err := svc.GetValidation(val)
	assert.NoError(t, err)
...
func TestService_SignalExit_SetExitBlock_Error(t *testing.T) {
	...
	assert.NoError(t, err)
	err = svc.SignalExit(id, 14, val.StartBlock()+val.Period()*current, 20)
	assert.Error(t, err)
}

func TestService_IncreaseStake_SuccessQueued(t *testing.T) {
	svc, _, _ := newSvc()
	idv := thor.BytesToAddress([]byte("q"))
	id := idv
	endorser := id
	assert.NoError(t, svc.repo.addValidation(id, &Validation{
		&body{
...
func TestService_IncreaseStake_SuccessActiveNoExit(t *testing.T) {
	svc, _, _ := newSvc()
	idv := thor.BytesToAddress([]byte("a"))
	id := idv
	endorser := id
	assert.NoError(t, svc.repo.addValidation(id, &Validation{
		&body{
...
func TestService_WithdrawStake_ClearCooldownWhenMatured(t *testing.T) {
	...
	idv := thor.BytesToAddress([]byte("ex"))
	id := idv
	endorser := id
	eb := uint32(10)
	assert.NoError(t, svc.repo.addValidation(id, &Validation{
		&body{
			...
			Status:          StatusExit,
			ExitBlock:       &eb,
			CooldownVET:     uint64(40),
			WithdrawableVET: uint64(5),
		},
	}))
...
func TestService_WithdrawStake_ActiveClearsQueuedAndWithdrawable(t *testing.T) {
	...
	idv := thor.BytesToAddress([]byte("act"))
	id := idv
	endorser := id
	assert.NoError(t, svc.repo.addValidation(id, &Validation{
		&body{
...
func TestService_Evict(t *testing.T) {
	svc, addr, st := newSvc()
	id1v := thor.BytesToAddress([]byte("id1"))
	id1 := id1v

	assert.NoError(t, svc.Add(id1, id1, thor.LowStakingPeriod(), uint64(1)))

	assert.NoError(t, svc.SignalExit(id1, 5, 5+thor.EpochLength(), int(thor.InitialMaxBlockProposers)))
	...
	assert.Error(t, svc.SignalExit(id1, 7, 7+thor.EpochLength(), int(thor.InitialMaxBlockProposers)))

	poisonValidationSlot(st, addr, id1)
	assert.Error(t, svc.SignalExit(id1, 8, 8+thor.EpochLength(), int(thor.InitialMaxBlockProposers)))
}
...
func TestService_Renew(t *testing.T) {
	...
	assert.Equal(t, uint64(0), val.PendingUnlockVET())
	assert.Equal(t, uint64(650), val.QueuedVET())
	assert.Equal(t, uint64(0), val.CooldownVET())
	assert.Equal(t, uint64(0), val.WithdrawableVET())
	...
	assert.NoError(t, err)
	assert.Equal(t, uint64(650), delta.LockedIncrease.VET)
	assert.Equal(t, uint64(1300), delta.LockedIncrease.Weight)
	assert.Equal(t, uint64(650), delta.QueuedDecrease)

	val, err = svc.GetValidation(id1)
	assert.NoError(t, err)
	assert.Equal(t, uint64(650), val.LockedVET())
	assert.Equal(t, uint64(2800), val.Weight())
	assert.Equal(t, uint64(0), val.PendingUnlockVET())
	assert.Equal(t, uint64(0), val.QueuedVET())
	assert.Equal(t, uint64(0), val.CooldownVET())
	assert.Equal(t, uint64(0), val.WithdrawableVET())
	...
	assert.Equal(t, uint64(2800), val.Weight())
	assert.Equal(t, uint64(0), val.PendingUnlockVET())
	assert.Equal(t, uint64(400), val.QueuedVET())
	assert.Equal(t, uint64(0), val.CooldownVET())
	assert.Equal(t, uint64(0), val.WithdrawableVET())

	delta, err = svc.Renew(id1, uint64(1500))
	assert.NoError(t, err)
	assert.Equal(t, uint64(400), delta.LockedIncrease.VET)
	assert.Equal(t, uint64(800), delta.LockedIncrease.Weight)
	assert.Equal(t, uint64(400), delta.QueuedDecrease)

	val, err = svc.GetValidation(id1)
	assert.NoError(t, err)
	assert.Equal(t, uint64(1050), val.LockedVET())
	assert.Equal(t, uint64(1050*2+1500), val.Weight())
	assert.Equal(t, uint64(0), val.PendingUnlockVET())
	assert.Equal(t, uint64(0), val.QueuedVET())
	assert.Equal(t, uint64(0), val.CooldownVET())
	assert.Equal(t, uint64(0), val.WithdrawableVET())
