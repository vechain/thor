...
// This test reproduces the bug on master (without the lazy-populate fix):
// 1) Warm the cache without reward percentiles (cachedRewards stays nil)
// 2) Request with reward percentiles; rewards are incorrectly all zeros
func TestReproduceZeroRewardsAfterCacheWarmup(t *testing.T) {
	feesData, newestBlockSummary := preparePostGalacticaChain(t, 4)

	// Warm the cache WITHOUT reward percentiles: entries are cached with cachedRewards == nil
	_, _, _, _, err := feesData.resolveRange(newestBlockSummary, 3, nil)
	require.NoError(t, err)

	// Now request WITH reward percentiles: on buggy master code, rewards will be all zeros
	...
	// Expect all zeros due to the bug (despite transactions having positive priority fees)
	for i := range rewards {
		require.NotNil(t, rewards[i])
		require.Len(t, rewards[i], 3)
		for j := range rewards[i] {
			assert.Equal(t, "0x0", rewards[i][j].String(), "block %d reward %d should be 0 due to bug", i, j)
		}
	}
}

// This test shows the opposite behavior: warming the cache WITH percentiles
// results in subsequent calls WITH percentiles returning correct, non-zero rewards.
func TestRewardsAfterCacheWarmupWithPercentiles(t *testing.T) {
	feesData, newestBlockSummary := preparePostGalacticaChain(t, 4)

	// Warm the cache WITH reward percentiles: entries are cached with computed cachedRewards
	_, _, _, _, err := feesData.resolveRange(newestBlockSummary, 3, []float64{25, 50, 75})
	require.NoError(t, err)

	// Request again WITH reward percentiles: rewards should be non-zero and match expectations
	_, _, _, rewards, err := feesData.resolveRange(newestBlockSummary, 3, []float64{25, 50, 75})
	require.NoError(t, err)
...
// preparePostGalacticaChain creates a chain forked at GALACTICA=1 with a fixed dataset
// of dynamic fee transactions (priority fees 10 and 12). It returns the FeesData and
// the newest block summary so tests operate on identical data.
func preparePostGalacticaChain(t *testing.T, numberOfBlocks int) (*FeesData, *chain.BlockSummary) {
	t.Helper()
	forkConfig := thor.NoFork
	...
	return feesData, newestBlockSummary
}
