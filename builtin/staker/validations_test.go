...
func TestStaker_TotalStake(t *testing.T) {
	...
	for id, stake := range stakes {
		...
		onCoolDownTVL, onCoolDownTVLWeight, err := staker.validations.ExitValidator(id)
		require.NoError(t, err)
		...
		err = staker.globalStatsService.RemoveLocked(onCoolDownTVL, onCoolDownTVLWeight, aggExit)
		require.NoError(t, err)
...
func TestStaker_TotalStake_Withdrawal(t *testing.T) {
	...
	// todo wrap this up / use a validation service or staker api instead of direct calling and checking totals
	onCoolDownTVL, onCoolDownTVLWeight, err := staker.validations.ExitValidator(addr)
	require.NoError(t, err)
...
func TestStaker_RemoveValidator_NonExistent(t *testing.T) {
	...
	_, _, err := staker.validations.ExitValidator(datagen.RandAddress())
	assert.NoError(t, err)
}
...
func TestStaker_AddValidator_CannotAddValidationWithSameMasterAfterExit(t *testing.T) {
	...
	endorsor := datagen.RandAddress()
	err := staker.AddValidator(endorsor, master, uint32(360)*24*15, MinStake)
	assert.NoError(t, err)
	...
	_, _, err = staker.validations.ExitValidator(master)
	assert.NoError(t, err)
