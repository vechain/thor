...
func (a *Authority) First() (thor.Address, error) {
	...
	if err := a.contract.Operation("first").Call().Into(&out); err != nil {
...
func (a *Authority) Next(nodeMaster thor.Address) (thor.Address, error) {
	out := new(common.Address)
	if err := a.contract.Operation("next", common.Address(nodeMaster)).Call().Into(&out); err != nil {
		return thor.Address{}, err
	}
	return thor.Address(*out), nil
}
...
func (a *Authority) Executor() (thor.Address, error) {
	out := new(common.Address)
	if err := a.contract.Operation("executor").Call().Into(&out); err != nil {
		return thor.Address{}, err
	}
	return thor.Address(*out), nil
}
...
func (a *Authority) Get(nodeMaster thor.Address) (*AuthorityNode, error) {
	...
	if err := a.contract.Operation("get", common.Address(nodeMaster)).Call().Into(&out); err != nil {
		return nil, err
	}
...
func (a *Authority) Add(nodeMaster, endorsor thor.Address, identity thor.Bytes32) bind.OperationBuilder {
	return a.contract.Operation("add", common.Address(nodeMaster), common.Address(endorsor), common.Hash(identity))
}

// Revoke revokes an authority node
func (a *Authority) Revoke(nodeMaster thor.Address) bind.OperationBuilder {
	return a.contract.Operation("revoke", common.Address(nodeMaster))
}
