...
func (dummy) Opt(a string, b *int) (string, error)            { return a, nil } // b optional (pointer)
...
func TestRegisterNameValidation(t *testing.T) {
	var r registry
	assert.Error(t, r.registerNamespace("", dummy{}))         // empty namespace
	assert.Error(t, r.registerNamespace("empty", struct{}{})) // no suitable methods
	require.NoError(t, r.registerNamespace("dummy", dummy{}))
}
...
func TestParseArgs(t *testing.T) {
	...
	_, err = echo.parseArgs(json.RawMessage(`[]`))
	require.Error(t, err)
	assert.Equal(t, errcodeInvalidParams, err.(*jsonError).Code)

	// missing optional (pointer) trailing arg -> nil, no error
	opt := r.callback("dummy_opt")
	args, err = opt.parseArgs(json.RawMessage(`["x"]`))
	require.NoError(t, err)
	require.Len(t, args, 2)
	assert.Equal(t, "x", args[0].String())
