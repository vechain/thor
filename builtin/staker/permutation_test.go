...
type Action interface {
	Name() string
	Execute(ctx *ExecutionContext, s *StakerTest, blk int) error
	Check(ctx *ExecutionContext, s *StakerTest, blk int) error
	Next() []Action
...
type builtAction struct {
	name                    string
	execute                 func(*ExecutionContext, *StakerTest, int) error
	check                   func(*ExecutionContext, *StakerTest, int) error
	next                    []Action
	minParentBlocksRequired *int
}
...
func (a *builtAction) Execute(ctx *ExecutionContext, s *StakerTest, blk int) error {
	if a.execute == nil {
		return nil
	}
...
func (a *builtAction) Check(ctx *ExecutionContext, s *StakerTest, blk int) error {
	if a.check == nil {
		return nil
	}
...
func (b *ActionBuilder) WithExecute(fn func(*ExecutionContext, *StakerTest, int) error) *ActionBuilder {
	b.execute = fn
	return b
}

func (b *ActionBuilder) WithCheck(fn func(*ExecutionContext, *StakerTest, int) error) *ActionBuilder {
	b.check = fn
	return b
}
...
func RunWithResultTree(s *StakerTest, action Action, currentBlk int) error {
	log := createLogger()
...
func RunnerWithResults(s *StakerTest, action Action, currentBlk int, results map[string]*ActionResult, ctx *ExecutionContext, blockSequence map[int]int) error {
	log := createLogger()
...
func runHousekeepingForBlockRange(s *StakerTest, start, end int, log *slog.Logger) error {
	...
	for cblk := first; cblk <= end; cblk += interval {
		...
		_, err := s.Staker.Housekeep(uint32(cblk))
		if err != nil {
...
// cloneStaker creates a deep copy of the StakerTest with cloned state
func cloneStaker(ts *StakerTest) *StakerTest {
	...
	clonedStaker := New(addr, clonedState, params.New(addr, clonedState), nil)
...
func TestRandomPermutationManager(t *testing.T) {
	// Only run test if RUN_PERMUTATIONS is set.
