...
func TestBlockReader(t *testing.T) {
	...
	assert.Equal(t, []*chain.ExtendedBlock{
		{block.Compose(b3.Header(), b3.Transactions(), b3.Committee()), false},
		{block.Compose(b4.Header(), b4.Transactions(), b4.Committee()), false}},
		blks)
}
...
func TestBlockReaderFork(t *testing.T) {
	...
	assert.Equal(t, []*chain.ExtendedBlock{
		{block.Compose(b2x.Header(), b2x.Transactions(), b2x.Committee()), true},
		{block.Compose(b2.Header(), b2.Transactions(), b2.Committee()), false},
		{block.Compose(b3.Header(), b3.Transactions(), b3.Committee()), false},
		{block.Compose(b4.Header(), b4.Transactions(), b4.Committee()), false}},
		blks)
}
