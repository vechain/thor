...
func TestLegacyTx(t *testing.T) {
	...
	assert.Equal(t, big.NewInt(150), trx.EffectiveGasPrice(big.NewInt(100), nil))
...
func TestEffectivePriorityFeePerGas(t *testing.T) {
	...
	effectivePriorityFeePerGas, err := EffectivePriorityFeePerGas(common.Big0, overallGasPrice, overallGasPrice)
	assert.Nil(t, err)
	// Should return the overallGasPrice
	assert.Equal(t, effectivePriorityFeePerGas, overallGasPrice)
	...
	// baseFee < maxFee - maxPriorityFee
	effectivePriorityFeePerGas, err = EffectivePriorityFeePerGas(b1, trx.MaxPriorityFeePerGas(), trx.MaxFeePerGas())
	assert.Nil(t, err)
	// should be the maxPriorityFeePerGas
	assert.Equal(t, effectivePriorityFeePerGas, trx.MaxPriorityFeePerGas())

	// baseFee = maxFee - maxPriorityFee
	effectivePriorityFeePerGas, err = EffectivePriorityFeePerGas(b5, trx.MaxPriorityFeePerGas(), trx.MaxFeePerGas())
	assert.Nil(t, err)
	// should be the maxPriorityFeePerGas
	assert.Equal(t, effectivePriorityFeePerGas, trx.MaxPriorityFeePerGas())
	...
	effectivePriorityFeePerGas, err = EffectivePriorityFeePerGas(baseFee, trx.MaxPriorityFeePerGas(), trx.MaxFeePerGas())
	assert.Nil(t, err)
	// should be the maxPriorityFeePerGas
	assert.Equal(t, effectivePriorityFeePerGas, new(big.Int).Sub(trx.MaxFeePerGas(), baseFee))

	// errors
	_, err = EffectivePriorityFeePerGas(big.NewInt(11), trx.MaxPriorityFeePerGas(), trx.MaxFeePerGas())
	assert.Equal(t, err, ErrGasPriceLessThanBaseFee)

	_, err = EffectivePriorityFeePerGas(b5, trx.MaxFeePerGas(), trx.MaxPriorityFeePerGas())
	assert.Equal(t, err, ErrMaxFeeLessThanPriorityFee)
}
