...
func TestPackAfterGalacticaFork(t *testing.T) {
	...
	expectedErrorMessage := fmt.Sprintf("tx not adoptable now: %s: expected 10000000000000 got 9999999999999", fork.ErrMaxFeePerGasTooLow)
...
func TestAdoptErrorAfterGalactica(t *testing.T) {
	...
	tr = tx.MustSign(tr, genesis.DevAccounts()[0].PrivateKey)
	err = chain.MintBlock(genesis.DevAccounts()[0], tr)
	expectedErrMsg = fmt.Sprintf("unable to adopt tx into block: tx not adoptable now: %s: expected 10000000000000 got 9999999999999", fork.ErrMaxFeePerGasTooLow)
	assert.Equal(t, expectedErrMsg, err.Error())
