...
func nodeFromRPC(sender *net.UDPAddr, rn rpcNode) (*Node, error) {
	...
	n := NewNode(rn.ID, rn.IP, rn.UDP, rn.TCP, time.Now())
...
func ListenUDP(priv *ecdsa.PrivateKey, conn conn, realaddr *net.UDPAddr, nodeDBPath string, netrestrict *netutil.Netlist, unhandled chan ReadPacket) (*Network, error) {
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte, unhandled chan ReadPacket) error {
	pkt := ingressPacket{remoteAddr: from}
	if err := decodePacket(buf, &pkt); err != nil {
		// TODO: should this check be moved into decodePacket?
		unhandled <- ReadPacket{
			Data: buf,
			Addr: from,
		}
		return err
