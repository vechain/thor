...
func TestPackAfterGalacticaFork(t *testing.T) {
	...
	expectedErrorMessage := "tx not adoptable now: max fee per gas is less than block base fee: expected 10000000000000 got 9999999999999"
...
func TestAdoptErrorAfterGalactica(t *testing.T) {
	...
	tr = tx.MustSign(tr, genesis.DevAccounts()[0].PrivateKey)
	err = chain.MintBlock(genesis.DevAccounts()[0], tr)
	expectedErrMsg = "unable to adopt tx into block: tx not adoptable now: max fee per gas is less than block base fee: expected 10000000000000 got 9999999999999"
	assert.Equal(t, expectedErrMsg, err.Error())
