...
func (f *Flow) Pack(privateKey *ecdsa.PrivateKey, newBlockConflicts uint32, voteCOM bool) (*block.Block, *state.Stage, tx.Receipts, error) {
	...
	if f.Number() >= f.packer.forkConfig.FINALITY && voteCOM {
		builder.COM()
	}
