...
func TestContract(t *testing.T) {
	...
	output, _, _ := env.Create(origin, input, math.MaxUint64, new(big.Int))
	...
	output, _, _ = env.Call(origin, contractAddr, left, math.MaxUint64, new(big.Int))
	if output.VMErr != nil {
		t.Fatal("didn't expect error:", output.VMErr)
	}
	...
	output, _, _ = env.Call(origin, contractAddr, right, math.MaxUint64, new(big.Int))
	if output.VMErr != nil {
		t.Fatal("didn't expect error:", output.VMErr)
	}
	...
	output, _, _ = env.Call(origin, contractAddr, add, math.MaxUint64, new(big.Int))
	if output.VMErr != nil {
		t.Fatal("didn't expect error:", output.VMErr)
	}
