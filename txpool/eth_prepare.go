...
// ethPreparation is the immutable result of chain/state work for one
// transaction. It is computed without holding ethPoolMap.lock and applied only
// while committing a map mutation.
...
func newEthSenderView(sender *ethSender, stateNonce uint64) *ethSenderView {
	...
	maps.Copy(view.pending, sender.pending)
	maps.Copy(view.queue, sender.queue)
	return view
}
...
func (m *ethPoolMap) addPreparationObjects(
	txObj *TxObject,
	stateNonce uint64,
	pendingLimit int,
	priceBump uint64,
...
func (m *ethPoolMap) syncPreparationObjects(
...
func (m *ethPoolMap) washPreparationObjects(
	stateNonces map[thor.Address]uint64,
...
func (m *ethPoolMap) forkPreparationObjects(
