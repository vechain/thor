...
func MigrateSQLiteToPebbleCursorBasedBlockRange(sqlitePath, pebblePath string, fromBlock, toBlock uint32, opts *MigrationOptions) (*MigrationStats, error) {
	...
	if err := migrateMetadata(sqliteDB, pebbleDB, opts); err != nil {
...
func migrateEventsCursorBasedBlockRange(sqlitePath string, target logdb.LogDB, fromBlock, toBlock uint32, totalEvents int64, opts *MigrationOptions) (int64, error) {
	...
	var writer logdb.Writer
	if pebbleDB, ok := target.(*pebbledb.PebbleV3LogDB); ok {
...
func migrateTransfersCursorBasedBlockRange(sqlitePath string, target logdb.LogDB, fromBlock, toBlock uint32, totalTransfers int64, opts *MigrationOptions) (int64, error) {
