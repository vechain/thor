...
func TestProtocolHandshake(t *testing.T) {
	...
	go func() {
		...
		// thor's DiscReason is uint8 (upstream: uint), so rlp's byte-string
		// fast path would kick in for a []DiscReason and mis-encode the
		// expectation; []any matches what SendItems actually put on the wire.
...
func TestRLPXFrameRW(t *testing.T) {
	...
	for i := range 10 {
		...
		wmsg := []any{"foo", "bar", strings.Repeat("test", i)}
...
// tcpPipe creates an in-process full duplex pipe over a localhost TCP socket.
...
func TestHandshakeSizeLimit(t *testing.T) {
	...
	// Provide only plainSize bytes: the 2-byte prefix declares an oversized message, the rest is garbage.
	buf := make([]byte, encAuthMsgLen)
	binary.BigEndian.PutUint16(buf[:2], 2049)
	_, err = readHandshakeMsg(new(authMsgV4), encAuthMsgLen, prv, bytes.NewReader(buf))
	if err == nil || err.Error() != "handshake message too big" {
		t.Fatalf("size=2049: want \"handshake message too big\", got %v", err)
	}
	// Boundary: 2048 must not trip the limit (it proceeds to the follow-up read and fails with an EOF-class error, not the size error).
	binary.BigEndian.PutUint16(buf[:2], 2048)
	_, err = readHandshakeMsg(new(authMsgV4), encAuthMsgLen, prv, bytes.NewReader(buf))
	if err == nil || err.Error() == "handshake message too big" {
		t.Fatalf("size=2048 must not trip the limit, got %v", err)
	}
}
