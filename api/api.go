...
func New(chain *chain.Chain, stateCreator *state.Creator, txPool *txpool.TxPool, logDB *logdb.LogDB, nw node.Network, allowedOrigins string, blockOffset uint32) (http.HandlerFunc, func()) {
	...
	node.New(nw).
		Mount(router, "/node")
	subs := subscriptions.New(chain, origins, blockOffset)
