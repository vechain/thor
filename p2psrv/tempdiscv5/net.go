...
func (net *Network) internNode(pkt *ingressPacket) *Node {
	...
	n := NewNode(pkt.remoteID, pkt.remoteAddr.IP, uint16(pkt.remoteAddr.Port), uint16(pkt.remoteAddr.Port), time.Now())
	n.state = unknown
...
func (net *Network) internNodeFromDB(dbn *Node) *Node {
	...
	n := NewNode(dbn.ID, dbn.IP, dbn.UDP, dbn.TCP, dbn.AddedAt)
	n.state = unknown
	net.nodes[n.ID] = n
	return n
}
