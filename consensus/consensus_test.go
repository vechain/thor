...
func newTestConsensus() (*testConsensus, error) {
	...
	flow, err := p.Schedule(parentSum, parent.Header().Timestamp()+100*thor.BlockInterval, 0)
	if err != nil {
		return nil, err
	}
	...
	flow2, err := p2.Schedule(b1sum, b1.Header().Timestamp()+100*thor.BlockInterval, 0)
	if err != nil {
		return nil, err
	}
