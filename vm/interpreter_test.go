...
func TestInterpreterMcopyOverlapping_Run(t *testing.T) {
	jt := NewDencunInstructionSet()
	interpreter := GetNewInterpreter(jt)

	// EIP-5656 test: MCOPY 0 1 8 (overlapping, src > dst)
	// Pre: 0x0001020304050607 08 00...
	// Post: 0x0102030405060708 08 00...
	byteCode := []byte{
		// Store 0x0001020304050607080000...00 at memory[0..31]
		byte(PUSH32),
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		...
		byte(PUSH1), 0x00,
		byte(MSTORE),
		byte(PUSH1), 0x08, // length = 8
		byte(PUSH1), 0x01, // src = 1
		byte(PUSH1), 0x00, // dst = 0
		byte(MCOPY),
		byte(PUSH1), 0x20, // return size = 32
		byte(PUSH1), 0x00, // return offset = 0
