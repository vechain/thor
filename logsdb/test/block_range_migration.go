...
func MigrateSQLiteToPebbleCursorBasedBlockRange(sqlitePath, pebblePath string, fromBlock, toBlock uint32, opts *MigrationOptions) (*MigrationStats, error) {
	...
	sqlite3, err := sqlite3.New(sqlitePath)
	if err != nil {
	...
	if opts.VerifyData {
		...
		if err := verifyMigration(sqlite3, pebbleDB, opts); err != nil {
...
func migrateEventsCursorBasedBlockRange(sqlitePath string, target logsdb.LogsDB, fromBlock, toBlock uint32, totalEvents int64, opts *MigrationOptions) (int64, error) {
	...
	if pebbleDB, ok := target.(*pebbledb.PebbleLogsDB); ok {
	...
	for {
		...
		// Process batch with same structure as sqlite3
		for rows.Next() {
			...
			// Convert binary data to proper types (same as sqlite3 queryEvents method)
...
func migrateTransfersCursorBasedBlockRange(sqlitePath string, target logsdb.LogsDB, fromBlock, toBlock uint32, totalTransfers int64, opts *MigrationOptions) (int64, error) {
