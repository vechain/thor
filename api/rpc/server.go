...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeEnvelope(w, env)
		s.logExchange("(unknown)", referer, origin, env, body, time.Since(start))
		return
	}

	env := s.dispatchAndLog(r.Context(), referer, origin, body, start)
	writeEnvelope(w, env)
}
...
func (s *Server) dispatchAndLog(ctx context.Context, referer, origin string, body []byte, start time.Time) (env rpcResponse) {
	method := "(unknown)"
	...
	defer func() { s.logExchange(method, referer, origin, env, previewSrc, time.Since(start)) }()
